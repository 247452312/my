...
public class KproUtil {
    ...
    private static HashMap<String, String> getFileHashMapByTableInfos(HashMap<String, TableInfo> tableInfos, DbInformation dbInformation, DbTypeEnum typeEnum) {
        ...
        String servicePath = "my-service/my-service-" + dbInformation.getDbName();
    ...
    private static Map<String, String> createDockerfile(String servicePath, DbInformation dbInformation) {
        ...
        HashMap<String, String> result = new HashMap<>(1);
        StringBuilder sb = new StringBuilder();
        sb.append("# Docker image for {tomcat? springBoot?}\n").append("# VERSION {版本号}\n").append("# Author: uhyils\n").append("# docker stop prod-my-service-").append(dbInformation.getDbName()).append("\n").append("# docker rm prod-my-service-").append(dbInformation.getDbName()).append("\n").append("# docker rmi my-service/prod-my-service-").append(dbInformation.getDbName()).append(":latest\n").append("# docker build --build-arg version=0.0.3 --build-arg runEnv=prod -t my-service/prod-my-service-").append(dbInformation.getDbName()).append(" .\n").append("# docker run -it -d  -p 8053:8053 -p 20853:20853 --name prod-my-service-").append(dbInformation.getDbName()).append(" my-service/prod-my-service-").append(dbInformation.getDbName()).append(":latest java -jar /app.jar --spring.profiles.active=prod\n").append("\n").append("FROM java:8\n").append("\n").append("ARG runEnv\n").append("ARG version\n").append("\n").append("#作者\n").append("MAINTAINER uhyils <uhyils@qq.com>\n").append("\n").append("#应用构建成功后的jar文件被复制到镜像内，名字也改成了app.jar\n").append("ADD target/my-service-").append(dbInformation.getDbName()).append("-$version.jar /app.jar\n").append("\n").append("#启动容器时的进程\n").append("CMD [\"java\",\"-jar\",\"/app.jar\",\"--spring.profiles.active=$runEnv\"]\n");
