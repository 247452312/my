...
public class KproUtil {
    ...
    private static Map<? extends String, ? extends String> createHtml(String filepath, String className, TableInfo tableInfo, String dateFormat) {
        ...
        tableInfo.getColums().entrySet().forEach(t -> {
            ...
            String smallName = value.getSmallName();
            String remark = value.getRemark();
            String mark = "    <div class=\"layui-form-item\">\n" +
                    "        <label class=\"layui-form-label required\">" + remark + "</label>\n" +
                    "        <div class=\"layui-input-block\">\n" +
                    "            <select name=\"" + smallName + "\" lay-filter=\"aihao\">\n" +
                    "                <option value=\"\">请选择" + remark + "</option>\n" +
                    "            </select>\n" +
                    "        </div>\n" +
                    "    </div>\n";
            detailCols.append(mark);
        });
        ...
        detailHtmlSb.append("<!DOCTYPE html>\n" +
                ...
                "\n" +
                "        if (func == 'edit') { // 这是修改\n" +
                "            pushRequest(\"" + className + "Service\", \"getById\", {\n" +
                "                id: id\n" +
                "            }, function (data) {\n" +
                "                $('[name=\"id\"]').attr('value', data.id);\n" +
                detailEditCols.toString() +
                "                form.render();\n" +
                "            })\n" +
                "        }\n" +
                "\n" +
                ...
                "                layer.close(index);\n" +
                "                if (func === 'add') {\n" +
                "                    pushRequest(\"" + className + "Service\", \"insert\", {data: data.field}, function () {\n" +
                "                    });\n" +
                "                    closeAndReload();\n" +
                "                } else if (func === 'edit') {\n" +
                "                    pushRequest(\"" + className + "Service\", \"update\", {data: data.field}, function () {\n" +
                "                    });\n" +
                "                    closeAndReload();\n" +
                "                }\n" +
                "            });\n" +
