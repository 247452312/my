...
public class KproUtil {
    ...
    private static Map<String, String> createDockerfile(String servicePath, DbInformation dbInformation) {
        ...
        HashMap<String, String> result = new HashMap<>(1);
        StringBuilder sb = new StringBuilder();
        sb.append("# Docker image for {tomcat? springBoot?}\n").append("# VERSION {版本号}\n").append("# Author: uhyils\n").append("# docker stop prod-my-service-user\n").append("# docker rm prod-my-service-user\n").append("# docker rmi my-service/prod-my-service-user:latest\n").append("# docker build --build-arg version=0.0.6-my-SNAPSHOT --build-arg runEnv=prod -t my-service/prod-my-service-user .\n").append("# docker run -it -d  -p ").append(dbInformation.getPort() + 50).append(":").append(dbInformation.getPort() + 50).append(" -p 20852").append(dbInformation.getPort() + 12850).append(":").append(dbInformation.getPort() + 12850).append(" -v /my/logs:/my/logs --name prod-my-service-user my-service/prod-my-service-user:latest\n").append("\n").append("FROM java:8\n").append("\n").append("#作者\n").append("MAINTAINER uhyils <uhyils@qq.com>\n").append("\n").append("\n").append("#挂载\n").append("VOLUME /my/logs\n").append("\n").append("#暴露端口\n").append("EXPOSE ").append(dbInformation.getPort() + 50).append(" ").append(dbInformation.getPort() + 12850).append("\n").append("\n").append("#应用构建成功后的jar文件被复制到镜像内，名字也改成了app.jar\n").append("ADD target/my-*.jar /app.jar\n").append("\n").append("#启动容器时的进程\n").append("ENTRYPOINT [\"java\",\"-Xms512m\",\"-Xmx512m\",\"-XX:+PrintGCTimeStamps\",\"-XX:+UseG1GC\",\"-XX:+HeapDumpOnOutOfMemoryError\",\"-XX:HeapDumpPath=/my/logs/").append(dbInformation.getSmallProjectName()).append("/JVM_DOWN.dump\",\"-XX:NewRatio=1\",\"-Xloggc:/my/logs/").append(dbInformation.getSmallProjectName()).append("/JVM.log\",\"-jar\",\"/app.jar\",\"--spring.profiles.active=prod\"]\n");
